...
blast_command_final = "usearch -ublast %s -db %s -evalue 1e-5 -target_cov 0.60 -matched %s -blast6out %s -threads %s -strand both -qsegout %s"
...
bridger_command_cov = "Bridger.pl --seqType fq --left %s --right %s --CPU %s --clean --min_kmer_coverage %s --output %s --pair_gap_length %s --kmer_length %s"
bridger_command = "Bridger.pl --seqType fq --left %s --right %s --CPU %s --clean --output %s --pair_gap_length %s --kmer_length %s"
blast_filter = "usearch -ublast %s -db %s -evalue 1e-5 -target_cov 0.50 -threads %s -matched %s -strand both"
...
if len(arguments) == 1:
	sys.exit("module load python usearch bridger")
try:
...
#			rename_final_out.write(">" + name + "_" + line2 + "__" + taxaname + "\n")
	...
	while line:
		if line[0] == ">":
			line2 = line.lstrip(">").strip("\n")
#			final_probe_out.write(">" + name + "_" + line2 + "__" + taxaname + "\n")
...
#os.system(remove_command % ("*.tfastq"))
